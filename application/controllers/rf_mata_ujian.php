...
class rf_mata_ujian extends MY_Controller {
	...
	public function index()
	{
        ...
        //Load Modal
        $this->mydata['modal']=$this->load->view('rf/mata_ujian/modal_input',$this->mydata,true);
        
        $this->mydata['content']=$this->load->view('rf/mata_ujian/content',$this->mydata,true);
        $this->load->view('main_page',$this->mydata);
	}
    
    public function retrieveData(){
        $this->load->library('ssp');
        $table = 'vpmb_rf_mata_ujian';
         
        // Table's primary key
        $primaryKey = 'Id_Mata_Ujian';
         
        $columns = array(
            array( 'db' => 'Id_Mata_Ujian', 'dt' => 0 ),
            array( 'db' => 'Mata_Ujian',  'dt' => 1 ),
    ...
    public function changeAktif(){
        ...
        $res=$this->app_model->changeAktif('tb_pmb_rf_mata_ujian','Id_Mata_Ujian',$id);
    ...
    public function simpan()
	{
	   $p=$this->input->post();
        $data=$p;
        $data=mappingColumn("tb_pmb_rf_mata_ujian",$data);
        $data=array_filter($data);
        if($p['saveas']=='baru'){
            $data['isAktif']='YES';
            $data=$this->addInsertLog($data);
            $res['isSuccess']=$this->db->insert("tb_pmb_rf_mata_ujian",$data);   
        }else{
            $key = array(
                'Id_Mata_Ujian' => $data['Id_Mata_Ujian'],
            );
            $data=$this->addUpdateLog($data);
            $res['isSuccess']=$this->db->update('tb_pmb_rf_mata_ujian',$data,$key);
        }		
    ...
    public function hapus(){
        $id=$this->input->post('id');
        $res=$this->app_model->hapusMataUjian($id);
