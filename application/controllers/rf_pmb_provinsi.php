...
class rf_pmb_provinsi extends MY_Controller {
	...
	public function index()
	{
        ...
        //Load Modal
        $this->mydata['modal']=$this->load->view('rf/provinsi/modal_input',$this->mydata,true);
        
        $this->mydata['content']=$this->load->view('rf/provinsi/content',$this->mydata,true);
        $this->load->view('main_page',$this->mydata);
	}
    
    public function retrieveData(){
        $this->load->library('ssp');
        $table = 'vpmb_rf_provinsi';
         
        // Table's primary key
        $primaryKey = 'Kode_Prop';
         
        $columns = array(
            array( 'db' => 'Kode_Prop', 'dt' => 0 ),
    ...
    public function changeAktif(){
        ...
        $res=$this->app_model->changeAktif('tb_akd_rf_propinsi','Kode_Prop',$id);
    ...
    public function simpan()
	{
	   $p=$this->input->post();
        $data=$p;
        $data=mappingColumn("tb_akd_rf_propinsi",$data);
        if($p['saveas']=='baru'){
            $data['Kode_Prop']=$this->app_model->getNewIdProvinsi();
            $data['isAktif']='YES';
            $data=$this->addInsertLog($data);
            $res['isSuccess']=$this->db->insert("tb_akd_rf_propinsi",$data);   
        }else{
            $key = array(
                'Kode_Prop' => $data['Kode_Prop'],
            );
            $data=$this->addUpdateLog($data);
            $res['isSuccess']=$this->db->update('tb_akd_rf_propinsi',$data,$key);
        }		
