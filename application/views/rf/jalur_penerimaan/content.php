...
<script type="text/javascript">
    ...
    function <?php echo $page_id;?>_tambah(){
        $('#<?php echo $page_id;?>ModalInputTitle').html('Tambah <?php echo $page_info['Nama_Menu']?>');
        $('#<?php echo $page_id;?>ModalInput').modal('show');
        setResetForm();
        $(".form_rf_jalur_penerimaan#saveas").val('baru');
    }
    function setResetForm(){
        reset_validation();
        clear_form('form_rf_jalur_penerimaan');
    	return false;
    }
    function setSyarat(id,text){
        var string="id="+id;
        $.ajax({
			type	: 'POST',
			url		: "<?php echo base_url(); ?>index.php/<?php echo $page_id;?>/setViewForDetailSyarat",
			data	: string,
            dataType : "json",
			cache	: false,
            success	: function(data){
                ...
                document.getElementById("det_Nama_JalurPenerimaan").innerHTML=text;
                //Table pada detail
                ...
                $('#<?php echo $page_id;?>_modal_setsyarat').modal('show');
			},
			error : function(xhr, teksStatus, kesalahan) {
				toastr['error']("<?php echo $page_info['Nama_Menu']?> tidak berhasil menampilkan detail", "<?php echo $page_info['Nama_Menu']?>")
				return false;
			}
		}); 
    }
    
    function setBerkas(id,text){
        $.post( "<?php echo base_url(); ?>index.php/<?php echo $page_id;?>/setViewForDetailBerkas",{id:id}, function( data ){
            $("#form_berkas_id_jalur").val(id);
            $("#form_syarat_berkas").html(data.optSyarat);
            $("#berkas_Nama_JalurPenerimaan").html(text);
            //Table pada detail
            var table = $('#dataDetBerkas<?php echo $page_id;?>').dataTable().fnDestroy();
            
            var table = $('#dataDetBerkas<?php echo $page_id;?>').DataTable({
                "processing":true,
                "serverSide":true,
                "ajax":"<?php echo base_url()?>index.php/<?php echo $page_id;?>/retrieveDataBerkas"
            });
            ...
            $('#<?php echo $page_id;?>_modal_setBerkas').modal('show');
        }, "json" );
    }
</script>
