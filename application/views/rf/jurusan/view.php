			...
			<div class="col-md-12">
                <div class="portlet box green-haze">
                    ...
                    <div class="portlet-body">
        				<table id="dataJurusan" class="table table-striped table-bordered table-hover">
                           <thead>
                              <tr>
                                 <th class="sort-alpha">Id Jurusan</th>
                                 <th class="sort-alpha">Nama Jurusan</th>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        var table = $('#dataJurusan').DataTable({
            "processing":true,
            "serverSide":true,
            "ajax":"<?php echo base_url()?>index.php/rf_pmb_jurusan/retrieveData"
        });
    ...
    function reloadTable(){
        var table = $('#dataJurusan').DataTable().ajax.reload();
    }
    ...
    function setHapusJurusan(id,nama){
        bootbox.confirm("Anda yakin ingin menghapus jurusan "+nama+" ?", function(result) {
            if(result==true){
                var string="id="+id;
                $.ajax({
        			type	: 'POST',
        			url		: "<?php echo base_url(); ?>index.php/rf_pmb_jurusan/hapus",
        			data	: string,
        			cache	: false,
        			success	: function(data){
        				toastr['success']("Jurusan berhasil dihapus", "Jurusan")
                        reloadTable();
        			},
        			error : function(xhr, teksStatus, kesalahan) {
        				toastr['error']("Jurusan tidak berhasil dihapus", "Jurusan")
        				return false;
        			}
        		});  
            }
    ...
    function editJurusan(id,nama){
        $("#form_nama_jurusan").val(nama);
        $("#form_id_jurusan").val(id);
    ...
    function setChangeActive(id){
        ...
        $.ajax({
			type	: 'POST',
			url		: "<?php echo base_url(); ?>index.php/rf_pmb_jurusan/changeAktif",
			data	: string,
			cache	: false,
			success	: function(data){
				toastr['success']("Perubahan aktif/non-aktif tersimpan", "Jurusan");
                reloadTable();
			},
			error : function(xhr, teksStatus, kesalahan) {
                toastr['error']("Perubahan aktif/non-aktif gagal", "Jurusan");
				return false;
			}
		});
    }
